...
import { cancelReservation, fetchBikesByHub, fetchMyTrips, getPendingReservation, reserveBike } from "../../Controllers/MobileAppController.js";
...
mobileAppNonAuthRouter.get("/trip", (request: Request, response: Response, next: NextFunction) => {
    const customerRequest: CustomRequest = request as CustomRequest
    getPendingReservation(customerRequest, response).catch(next)
});
